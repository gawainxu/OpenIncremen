...
class RKAngle(nn.Module):
    def forward(self, last, current, label, old_targets):
        ...
        f1_current, f2_current = torch.split(current, [bsz, bsz], dim=0)
        f1_last, f2_last = torch.split(last, [bsz, bsz], dim=0)
        last = torch.cat([f1_last[m, :], f2_last[m, :]], dim=0)
        current = torch.cat([f1_current[m, :], f2_current[m, :]], dim=0)
